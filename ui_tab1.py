...
def render_tab1():
    ...
    # 출발지 정보와 도착지 정보 가로 정렬
    col_from_main, col_to_main = st.columns(2)

    with col_from_main:
        st.subheader("출발지 정보")
        from_addr_cols = st.columns([3,1])
        ...
        with from_addr_cols[1]:
            st.text_input("층수", key="from_floor", label_visibility="collapsed", placeholder="출발층 (예: 3)")
    ...
    with col_to_main:
        ...
        with to_addr_cols[1]:
            st.text_input("층수", key="to_floor", label_visibility="collapsed", placeholder="도착층 (예: 5)")
    ...
    if st.session_state.get('has_via_point'):
        with st.container(border=True):
            ...
            with via_addr_cols[0]:
                 st.text_input("경유지 주소", key="via_point_location", label_visibility="collapsed", placeholder="경유지 전체 주소")
    ...
    # 보관이사 UI는 이미 구현된 내용 유지 (render_tab1 함수 하단)
