...
def render_tab3():
    ...
    with st.container(border=True):
        ...
        with col_pdf_btn:
            if st.button("📄 고객용 PDF 생성", key="generate_customer_pdf_btn", disabled=actions_disabled or not pdf_generation_possible):
                ...
                if pdf_data:
                    ...
                    # PDF 기반 이미지도 함께 생성 시도 (선택적)
                    if pdf_to_image_possible:
                        with st.spinner("PDF 기반 고객용 이미지 생성 중..."):
                            # Poppler 경로를 환경 변수나 secrets에서 가져오도록 설정할 수 있습니다.
                            # 예: poppler_bin_path = st.secrets.get("poppler_bin_path")
                            poppler_bin_path = None # 필요한 경우 여기에 경로 지정
        ...
        with col_pdf_img_btn:
            # PDF 기반 이미지 생성 버튼 (만약 PDF 생성과 분리하고 싶다면)
            # if st.button("🖼️ PDF -> 이미지 변환 (고객용)", key="generate_pdf_to_image_btn", disabled=actions_disabled or not pdf_to_image_possible or not st.session_state.get('customer_final_pdf_data')):
            #    if st.session_state.get('customer_final_pdf_data'):
            #        with st.spinner("PDF 기반 고객용 이미지 생성 중..."):
            #            poppler_bin_path = None # 필요시 경로 지정
            #            img_data_from_pdf = pdf_generator.generate_quote_image_from_pdf(st.session_state['customer_final_pdf_data'], poppler_path=poppler_bin_path)
            #        if img_data_from_pdf:
            #            st.session_state['customer_pdf_image_data'] = img_data_from_pdf
            #            st.success("✅ PDF 기반 고객용 이미지 생성 완료!")
            #        else:
            #            st.error("❌ PDF 기반 고객용 이미지 생성 실패.")
            #            if 'customer_pdf_image_data' in st.session_state: del st.session_state['customer_pdf_image_data']
            #    else:
            #        st.warning("먼저 고객용 PDF를 생성해주세요.")
            ...
            elif pdf_to_image_possible and st.session_state.get('customer_final_pdf_data') and not actions_disabled : # PDF가 생성되어 있어야 변환 가능
                st.caption("PDF 생성 시 함께 생성됩니다. 또는 별도 변환 버튼(추가 시).")
    ...
    with st.container(border=True):
        ...
        company_form_image_args = {
            "state_data": st.session_state.to_dict(),
            "calculated_cost_items": st.session_state.get("calculated_cost_items_for_pdf", []),
            "total_cost_overall": st.session_state.get("total_cost_for_pdf", 0), # image_generator는 total_cost_overall 사용
            "personnel_info": st.session_state.get("personnel_info_for_pdf", {})
        }
    ...
    with st.container(border=True):
        ...
        if st.button("📧 이메일 발송", key="email_send_button_main_tab3", disabled=actions_disabled or not email_possible):
            ...
            pdf_email_bytes_send = st.session_state.get('customer_final_pdf_data') # 고객용 PDF 데이터 사용
            if not pdf_email_bytes_send and pdf_generation_possible: # PDF가 없으면 다시 생성
                ...
                if pdf_email_bytes_send: # 성공 시 세션에 저장
        ...
        elif not can_generate_anything and not actions_disabled : st.caption("견적 내용 확인 필요")
