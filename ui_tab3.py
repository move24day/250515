# ui_tab3.py (invalid syntax 오류 수정 및 보관이사 요약 포맷팅 적용)
...
def render_tab3():
    ...
    with st.container(border=True):
        ...
        if base_housewife_count_for_option > 0:
            st.checkbox(f"기본 여성 중 1명 제외 (할인: -{base_personnel_cost_one:,.0f}원, 현재 차량 기본 {base_housewife_count_for_option}명)", key="remove_base_housewife")
        else:
        ...
        if base_man_count_for_option > 0:
            st.checkbox(f"기본 남성 중 1명 제외 (할인: -{base_personnel_cost_one:,.0f}원, 현재 차량 기본 {base_man_count_for_option}명)", key="remove_base_man")
        else:
    ...
    with st.container(border=True):
        ...
        # ... (이전과 동일한 수기 조정 및 계약금 UI 로직, min_value=0 제거됨) ...
    ...
    st.header("최종 견적 결과")
    # ... (이전과 동일한 최종 견적 결과 계산 및 유효성 검사 로직) ...
        ...
        try:
            ...
            if summary_display_possible:
                try:
                    ...
                    men_summary = p_info_summary.get('final_men', 0); women_summary = p_info_summary.get('final_women', 0)
                    ...
                    bask_summary_str = ""; q_b_s, q_mb_s, q_book_s = 0,0,0
                    ...
                    is_tax_invoice_selected = st.session_state.get("issue_tax_invoice", False); is_card_payment_selected = st.session_state.get("card_payment", False)
                    ...
                    for name, cost, note in cost_items_display:
                        cost_int = 0; try: cost_int = int(float(cost or 0)); except: pass
                    ...
                    def build_summary_first_line(current_date_str, from_route_disp, to_route_disp, vehicle_tonnage_str, customer_email_str_param, is_tax_flag_param, has_via_flag_param_local, via_loc_str_for_route_param, via_floor_str_for_route_param, is_long_dist_flag_param, long_dist_selector_str_val_param, move_time_opt_str_val_param, afternoon_details_str_val_param):
                        ...
                        if is_long_dist_flag_param: ld_text = long_dist_selector_str_val_param;
                        if ld_text and ld_text != "선택 안 함": suffix_items_list.append(f"{ld_text} 장거리이사"); else: suffix_items_list.append("장거리이사")
                    ...
                    if is_storage_move_summary:
                        ...
                        summary_output_lines.append(f"총 (출발일 결제액) {payment_leg1_final:,.0f}원 중 (분할이사비 {common_costs_leg1_split:,.0f}원, 출발작업비 {departure_specific_costs_val:,.0f}원{dep_breakdown_vat_str})")
                        ...
                        # 아래 라인이 Syntax Error 발생 가능성 있는 지점 (line 490 근처)
                        # f-string 내부에 또 다른 f-string을 직접 결합하는 대신, 외부에서 문자열을 완성하고 삽입
                        joined_breakdown_str_leg2 = ", ".join(breakdown_parts_leg2)
                        summary_output_lines.append(f"총 (도착일 지불액) {payment_leg2_final:,.0f}원 중 ({joined_breakdown_str_leg2})")
                        ...
                        if formatted_line: cost_item_details_for_summary.append(formatted_line)
    ...
    with st.container(border=True):
        ...
        with col_pdf_btn:
            if st.button("고객용 PDF 생성", key="generate_customer_pdf_btn_tab3", disabled=actions_disabled or not pdf_generation_possible):
                ...
                if 'customer_final_pdf_data' in st.session_state and not pdf_data : del st.session_state['customer_final_pdf_data'] # 실패 시 키 제거
